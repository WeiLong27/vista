...
library Calculations {
     ...
     */
    function calculateCurrentYield(SP0VaultMetadata storage self) public {
        ...
        self.totalCouponPayoff = calculateCouponPayment(
     ...
     * @param vistaStateAddress is the address of the VistaState contract that stores the oracle addresses
     */
    function checkBarriers(
        SP0VaultMetadata storage self,
        address vistaStateAddress
    ) public {
        ...
        for (uint256 i = 0; i < self.optionBarriersCount; i++) {
            ...
            if (optionBarrier.barrierType == OptionBarrierType.KnockIn) {
                address oracle = getOracleAddress(
                    optionBarrier,
                    vistaStateAddress
                );
                (, int256 answer, uint256 startedAt, , ) = IOracle(oracle)
                    .latestRoundData();
                require(
                    block.timestamp - ORACLE_STALE_DELAY <= startedAt,
                    "400:T"
                );
     ...
     * @param self is the SP0VaultMetadata
     * @param vistaStateAddress is address of vistaState
     */
    function calculateVaultFinalPayoff(
        ...
        totalCouponPayment = calculateCouponPayment(
            self.underlyingAmount,
            self.aprBps,
            self.tenorInDays
        );
        ...
        // Calculate principal
        if (self.isKnockedIn) {
            principalToReturnBps = calculateKnockInRatio(
                self,
                vistaStateAddress
            );
        }
        ...
        totalPrincipal =
            (self.underlyingAmount * principalToReturnBps) /
            BPS_DECIMALS;
    ...
    function calculateKnockInRatio(
        ...
        for (uint256 i = 0; i < optionBarriersCount; i++) {
            ...
            address oracle = getOracleAddress(optionBarrier, vistaStateAddress);
            (, int256 answer, uint256 startedAt, , ) = IOracle(oracle)
                .latestRoundData();
            ...
            if (optionBarrier.barrierType == OptionBarrierType.KnockIn) {
                uint256 ratioBps = (uint256(answer) * LARGE_CONSTANT) /
    ...
    function calculateCouponPayment(
        ...
        return
            (underlyingAmount * daysPassed * aprBps * LARGE_CONSTANT) /
            DAYS_IN_YEAR /
            BPS_DECIMALS /
            LARGE_CONSTANT;
    }
     ...
     * @param vistaStateAddress is the address of the Vista state contract
     */
    function getOracleAddress(
        OptionBarrier memory optionBarrier,
        address vistaStateAddress
    ) private view returns (address) {
        IVistaState vistaState = IVistaState(vistaStateAddress);
        address oracle = vistaState.oracleAddresses(optionBarrier.oracleName);
