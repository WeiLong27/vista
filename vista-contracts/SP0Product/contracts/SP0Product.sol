...
import {IVistaState} from "./interfaces/IVistaState.sol";
import {Deposit, SP0VaultMetadata, OptionBarrierType, OptionBarrier, VaultStatus, Withdrawal} from "./Structs.sol";
import {SP0Vault} from "./SP0Vault.sol";
import {Calculations} from "./Calculations.sol";

contract SP0Product is ReentrancyGuard {
    using SafeERC20 for IERC20;
    using Calculations for SP0VaultMetadata;

    event SP0ProductCreated(
        address indexed vistaState,
    ...
    event OptionBarrierOracleUpdated(
        address indexed vaultAddress,
        uint256 index,
        string _asset,
        string _oracleName
    );
    event OptionBarrierRemoved(
        address indexed vaultAddress,
        uint256 index,
        string asset
    );
    ...
    event VaultFinalPayoffCalculated(
        address indexed vaultAddress,
        uint256 finalPayoffAmount,
        VaultStatus vaultStatus
    );

    event BarriersChecked(address indexed vaultAddress, bool isKnockedIn);

    event AssetsReceivedFromVistaState(
        address indexed vaultAddress,
        uint256 amount
    );
    ...
    mapping(address => SP0VaultMetadata) public vaults;
    ...
    /**
     * @notice Creates a new SP0Product
     * @param _vistaState is the address of the VistaState contract
    ...
    constructor(
        ...
        vistaState = IVistaState(_vistaState);
        asset = _asset;
        name = _name;
    ...
    modifier onlyDefaultAdmin() {
        require(vistaState.isDefaultAdmin(msg.sender), "403:DA");
        _;
    }
    ...
    modifier onlyTraderAdmin() {
        require(vistaState.isTraderAdmin(msg.sender), "403:TA");
        _;
    }
    ...
    modifier onlyOperatorAdmin() {
        require(vistaState.isOperatorAdmin(msg.sender), "403:OA");
        _;
    }

    /**
     * @notice Asserts that the vault has been initialized & is a Vista Vault
    ...
    ) public view returns (SP0VaultMetadata memory) {
     ...
     * @param _maxDepositAmountLimit is the deposit limit for the product
     */
    function setMaxDepositAmountLimit(
        uint256 _maxDepositAmountLimit
    ) public onlyTraderAdmin {
        require(
            queuedDepositsTotalAmount + sumVaultUnderlyingAmounts <=
                _maxDepositAmountLimit,
            "400:TooSmall"
        );
        maxDepositAmountLimit = _maxDepositAmountLimit;
    ...
    function createVault(
        ...
        SP0Vault vault = new SP0Vault(asset, _tokenName, _tokenSymbol);
        ...
        SP0VaultMetadata storage vaultMetadata = vaults[newVaultAddress];
    ...
    function setTradeData(
        ...
        SP0VaultMetadata storage metadata = vaults[vaultAddress];
    ...
    function addOptionBarrier(
        ...
        SP0VaultMetadata storage metadata = vaults[vaultAddress];
        require(
            metadata.vaultStatus == VaultStatus.DepositsClosed ||
                metadata.vaultStatus == VaultStatus.NotTraded,
            "500:WS"
        );
    ...
    function updateOptionBarrier(
        ...
        SP0VaultMetadata storage vaultMetadata = vaults[vaultAddress];

        require(_strikeAbsoluteValue > 0, "400:SV");
        require(_barrierAbsoluteValue > 0, "400:BV");

        OptionBarrier storage optionBarrier = vaultMetadata.optionBarriers[
            index
        ];
        require(
            keccak256(abi.encodePacked(optionBarrier.asset)) ==
        ...
        emit OptionBarrierUpated(
            vaultAddress,
            index,
            _asset,
            _strikeAbsoluteValue,
            _barrierAbsoluteValue
        );
    }
     ...
     * @param _asset is the ticker symbol of the asset we want to update
     * (included as a safety check since the asset name should match the option barrier at given index)
     * @param newOracleName is the name of the new oracle (must also register this name in VistaState)
     */
    function updateOptionBarrierOracle(
        ...
        require(
            vistaState.oracleAddresses(newOracleName) != address(0),
            "400:OR"
        );
        optionBarrier.oracleName = newOracleName;

        emit OptionBarrierOracleUpdated(
            vaultAddress,
            index,
            _asset,
            newOracleName
    ...
    function removeOptionBarrier(
        ...
        require(
            keccak256(abi.encodePacked(optionBarriers[index].asset)) ==
                keccak256(abi.encodePacked(_asset)),
            "400:AS"
        );
    ...
    function processDepositQueue(
        ...
        SP0Vault vault = SP0Vault(vaultAddress);
        require(
            !(vaultMetadata.underlyingAmount == 0 && vault.totalSupply() > 0),
            "500:Z"
        );
    ...
    function addToWithdrawalQueue(
        ...
        withdrawalQueue.push(
            Withdrawal({amountShares: amountShares, receiver: msg.sender})
        );
    ...
    function checkBarriers(
        ...
        SP0VaultMetadata storage vaultMetadata = vaults[vaultAddress];
        vaultMetadata.checkBarriers(address(vistaState));
    ...
    function calculateVaultFinalPayoff(
        ...
        SP0VaultMetadata storage vaultMetadata = vaults[vaultAddress];
        vaultFinalPayoff = vaultMetadata.calculateVaultFinalPayoff(
            address(vistaState)
        );
        emit VaultFinalPayoffCalculated(
            vaultAddress,
            vaultFinalPayoff,
            VaultStatus.PayoffCalculated
        );
    }
    ...
    function calculateKnockInRatio(
        ...
        SP0VaultMetadata storage vaultMetadata = vaults[vaultAddress];
        return vaultMetadata.calculateKnockInRatio(address(vistaState));
    }
    ...
    function receiveAssetsFromVistaState(
        ...
        require(msg.sender == address(vistaState), "403:CS");
        SP0VaultMetadata storage vaultMetadata = vaults[vaultAddress];

        IERC20(asset).safeTransferFrom(msg.sender, address(this), amount);
        vaultMetadata.currentAssetAmount += amount;

        emit AssetsReceivedFromVistaState(vaultAddress, amount);
    }
    ...
    function collectFees(
        ...
        SP0VaultMetadata storage vaultMetadata = vaults[vaultAddress];
        require(
            vaultMetadata.vaultStatus == VaultStatus.PayoffCalculated,
            "500:WS"
        );
        ...
        IERC20(asset).safeTransfer(vistaState.feeRecipient(), totalFees);
    ...
    function processWithdrawalQueue(
        ...
        uint256 processCount = Math.min(
            vaultMetadata.queuedWithdrawalsCount,
            maxProcessCount
        );
        uint256 amountAssets;
        ...
        while (processCount > 0) {
            ...
            vaultMetadata.queuedWithdrawalsSharesAmount -= withdrawal
                .amountShares;
        ...
        if (vaultMetadata.queuedWithdrawalsCount == 0) {
            if (
                ...
                vaultMetadata.vaultStatus = VaultStatus
    ...
    function sendAssetsToTrade(
        ...
        require(vistaState.marketMakerAllowList(receiver), "400:NotAllowed");
        SP0VaultMetadata storage vaultMetadata = vaults[vaultAddress];
        require(amount <= vaultMetadata.currentAssetAmount, "400:TooBig");
        IERC20(asset).safeTransfer(receiver, amount);
        vaultMetadata.currentAssetAmount =
            vaultMetadata.currentAssetAmount -
            amount;
        vaultMetadata.vaultStatus = VaultStatus.Traded;

        emit AssetsSentToTrade(
            vaultAddress,
            receiver,
            amount,
            VaultStatus.Traded
